...
Public Enum enumDivisibleRotation
    Angle180 = 180
    Angle90CW = -90
    Angle90CCW = 90
End Enum
...
'0-255, больше - светлее
Public Property Get GetColorLightness(ByVal Color As Color) As Long
    With Color.GetCopy
        .ConvertToHLS
        GetColorLightness = .HLSLightness
    End With
End Property

'0-255, больше - светлее
Public Property Get GetColorLuminance( _
                    ByVal Color As Color _
                ) As Long
    With Color.GetCopy
        .ConvertToLab
        GetColorLuminance = .LabLuminance
    End With
End Property
...
Public Property Get GreaterSize(ByVal Rect As Rect) As Double
    If Rect.Width > Rect.Height Then
        GreaterSize = Rect.Width
    Else
        GreaterSize = Rect.Height
    End If
End Property
...
Public Property Get GetDisplayCurve(ByVal MaybeShape As Variant) As Curve
    If Not IsShape(MaybeShape) Then GoTo Fail
    Dim Temp As Double
    On Error GoTo Fail
    Set GetDisplayCurve = MaybeShape.DisplayCurve
    On Error GoTo 0
Fail:
End Property
...
Public Property Get HasDisplayCurve(ByVal MaybeShape As Variant) As Boolean
    If Not IsShape(MaybeShape) Then GoTo Fail
    Dim Temp As Double
    On Error GoTo Fail
    Temp = MaybeShape.DisplayCurve.Length
    On Error GoTo 0
    HasDisplayCurve = Temp > 0
Fail:
End Property
...
Public Property Get LesserSize(ByVal Rect As Rect) As Double
    If Rect.Width < Rect.Height Then
        LesserSize = Rect.Width
    Else
        LesserSize = Rect.Height
    End If
End Property
...
'todo
Public Property Get FindFolderInGMSPath(ByVal FolderName As String) As String
    With FSO
        If .FolderExists(GMSManager.UserGMSPath & FolderName) Then
            'SetFolderInGMSPath = AddProperEndingToPath(GMSManager.UserGMSPath & FolderName)
        ElseIf .FolderExists(GMSManager.GMSPath & FolderName) Then
            'SetFolderInGMSPath = AddProperEndingToPath(GMSManager.GMSPath & FolderName)
        Else
            'IFileSystemHelper_SetFolderInGMSPath = AddProperEndingToPath(GMSManager.UserGMSPath & FolderName)
            '.CreateFolder SetFolderInGMSPath
        End If
    End With
End Property
...
Public Property Get CArray(ByRef Arr As Variant) As Variant()
    CArray = Arr
End Property
...
Public Sub Dummy( _
    ...
    If Module = "" Then _
            AdditionalInfo = Method _
        Else _
            AdditionalInfo = Module & "." & Method
    If Not AdditionalInfo = "" Then AdditionalInfo = ": " & AdditionalInfo
    Throw "ЗАГЛУШКА" & AdditionalInfo
End Sub
...
'то же, что и Assign, только через =
Public Property Let That(ByRef Result As Variant, ByRef RHS As Variant)
    If VBA.IsObject(RHS) Then Set Result = RHS Else Result = RHS
End Property
